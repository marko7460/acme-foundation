locals {
  nat_routers_tmp = flatten([for vpc_name, vpc_conf in var.shared_vpcs : [for nat_name, nat_conf in vpc_conf.nats : { network = vpc_name, nat_name = nat_name, region = nat_conf.region, conf = nat_conf }]])
  nat_routers     = { for nat_router in local.nat_routers_tmp : "${nat_router.network}-${nat_router.nat_name}-${nat_router.region}" => nat_router }
}
...
resource "google_compute_router" "router" {
  ...
  project     = module.vpc[each.value.network].project_id
  description = "Router for ${each.value.network} in ${each.value.region} for ${each.value.nat_name}"
...
resource "google_compute_router_nat" "nats" {
  ...
  dynamic "subnetwork" {
    ...
    content {
      name                     = "projects/${module.vpc[each.value.network].project_id}/regions/${each.value.region}/subnetworks/${subnetwork.value.name}"
